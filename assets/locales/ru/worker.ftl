...
bouncer-degraded-to-free-notification =
    ⏳ Срок действия VIP-доступа истек.

    Вы переведены на бесплатный базовый поток сигналов со стандартными фильтрами.
    Для возврата к персональным настройкам оформите подписку в меню /start.

bouncer-expired-unsubscribed-notification =
    ⏳ Срок действия VIP-доступа истек.

    Рассылка сигналов приостановлена, так как вы не подписаны на наш канал и чат сообщества.
    Подпишитесь на ресурсы ниже и нажмите кнопку проверки, чтобы продолжить получать бесплатные сигналы:

gate-button-channel = 📢 Новостной канал
gate-button-chat = 💬 Чат сообщества
gate-button-verify = 🔄 Проверить подписку
